...
using hls_int = ap_fixed<(8 + 5), 8, AP_RND>;
...
using hls_acc_int = ap_fixed<((8 + 5) + (8 + 5)), (8 + 5), AP_RND>;


__attribute__((sdx_kernel("ma", 0))) hls_int ma(hls_int sample);
# 2 "MA_HLS/ma.cpp" 2

hls_int line_buffer(hls_int const sample){
 static hls_int memory[32 +2];
 static ap_uint<8> j = 0;
#pragma HLS RESET variable=j

 memory[32 +1] = sample;
 VITIS_LOOP_9_1: for (int i=0; i<32 +1; i++){
#pragma HLS UNROLL
 memory[i] = memory[i+1];
 }
...
__attribute__((sdx_kernel("ma", 0))) hls_int ma(hls_int const sample){
#pragma HLS TOP name=ma
# 23 "MA_HLS/ma.cpp"
